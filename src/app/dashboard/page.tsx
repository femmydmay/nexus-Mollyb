...
interface Data extends Listing{
  Uploads:{url:string, id:string}[]
}
...
const listings = data as Data[];
  ...
  const handleSubmit = async (event:FormEvent) => { 
    ...
    try {
      ...
      const response = await axios.post("/api/email/verify/" + token, { email, code });
       if (response.status === 200) {
      ...
      {user?.email_verified ? (
        <div className="bg-white p-2 mt-10 h-full space-y-4">
          ...
          <div>
            ...
            <div className="border grid md:grid-cols-2 gap-3">
              {listings.map((listing) => (
                <div className="relative mx-auto w-full" key={listing.id}>
                  <Link
                    href={`/details/${listing.id}`}
                    className="relative inline-block duration-300 ease-in-out transition-transform transform hover:-translate-y-2 w-full"
                  >
                    <div className="shadow p-4 rounded-lg bg-white hover:bg-white/20">
                      <div className="flex justify-center relative rounded-lg overflow-hidden max-lg:h-52">
                        <div className="transition-transform duration-500 transform ease-in-out hover:scale-110 w-full">
                          <div className="h-40 inset-0  opacity-70 transition-all ease-in-out duration-100 hover:opacity-100 hover:bg-transparent">
                            <Image

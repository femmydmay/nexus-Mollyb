...
interface Items extends Listing {
  Uploads: { url: string; id: string }[];
}
const Products = ({ item, title }: { item: Items[]; title: string }) => {

  
  return (
    <section className=" my-10 bg-white pb-5">
      <header className={` py-4 pl-2 bg-slate-900 my-3`}>
        <p className="font-bold text-white">{title}</p>
      </header>
      <div className=" pro md:grid  md:grid-cols-2  lg:grid-cols-4 ml-4 overflow-hidden overflow-x-hidden gap-3">
        {item.length > 0 ?
          item.map((listing) => {
            return (
              <>
                {listing && (
                  <div className="relative mx-auto w-full border rounded">
                    <Link
